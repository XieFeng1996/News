...
-(void)InterStartContent{
        //初始化函数
        [self InterInit];
        NSString *strpage = [NSString stringWithFormat:@"http://api.tianapi.com/world/?key=bc2b1b37f98a7256e323175d0e01df6d&num=10"];
...
-(void)creatUItableView{
    _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, KW, KH-20) style:UITableViewStylePlain];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
   ...
   static NSString *cellName = @"ID";
    ...
    if (cell == nil) {
        ...
        mainLabel = (UILabel *)[cell.contentView viewWithTag:M_TAG];
        secondLabel = (UILabel *)[cell.contentView viewWithTag:S_TAG];
        photo = (UIImageView *)[cell.contentView viewWithTag:P_TAG];
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    //获得当前点击的URL
    InternationalModel *model = _arrayURL[indexPath.row];
    //传递给webView
...
-(void)scrollViewWillBeginDecelerating:(UIScrollView *)scrollView{
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0,0, KW, 80)];
    ...
    UIView *view2 = [[UIView alloc]initWithFrame:CGRectMake(0, 350, KW, 80)];
...
-(void)PullDownToRefresh:(NSInteger )page{
    NSString *https = [NSString stringWithFormat:@"http://api.tianapi.com/world/?key=bc2b1b37f98a7256e323175d0e01df6d&num=10&page=%ld",page];
...
-(void)upAnilysisJson:(NSDictionary *)response{
    ...
    for (NSInteger i=0; i<newslist.count; i++) {
        InternationalModel *model = [[InternationalModel alloc]init];
